    ...
    for lo, hi, part in PARTS:
        ...
        for n in range(lo, hi + 1):
            ...
            chapters.append("\\chapter{%s}\n\\input{bodies/topic_%02d}"
                            % (meta["title"], n))
    # The chapters cite the конспект's literature by number ([31], [42], ...).
    # Without the list itself those numbers address nothing, so it closes the book.
    chapters.append("\\chapter*{Литература по конспекта}\n"
                    "\\addcontentsline{toc}{chapter}{Литература по конспекта}\n"
                    "\\markboth{Литература по конспекта}{}\n"
                    "\\input{bodies/bibliography}")
...
Цветът казва какъв \emph{вид} обект е даден блок. Той не казва какво
\emph{изисква} конспектът --- дефиниция, само формулировка, пълно доказателство,
конструкция, псевдокод, реализация, задача или чертеж. Затова всяка глава
започва със сива рамка „По конспекта: какво трябва да мога“, в която всяко
изискване на официалната анотация е изписано с думи и посочва раздела, който
го изпълнява. Там стоят и официалните бележки кое ще бъде избрано на изпита
(въпроси 6, 7, 8 и 18), кое е предварителен материал, кое е допълнително и
къде покритието още не е пълно. Подробните списъци „Изпитен фокус“ в края на
главите остават за повторение.

Държавният изпит е в две части в два дни: практическа (решаване на задачи) и
теоретична, оценявани поотделно. Пълните указания са на стр. 3 на конспекта от
30.06.2025.

Номерата в квадратни скоби в бележките за източниците сочат официалната
литература на конспекта, възпроизведена в края на книгата.
